...
def clean_raw_data(data):
    ...
    blue_data = data[data['playerid']==100]
    blue_data.reset_index(drop=True, inplace=True)

    red_data = data[data['playerid']==200]
    ...
    for i in range(n_matches):
        ...
        if pd.isnull(prematch_blue_data['gameid'][i]):
            ...
            blue_game_slice = data[(data['date']==prematch_blue_data['gameid'][i]) &
                                   (data['side']=='Blue')]
            red_game_slice = data[(data['date']==prematch_red_data['gameid'][i]) &
                                  (data['side']=='Red')]
            
        # otherwise just use the gameid
        else:
            blue_game_slice = data[(data['gameid']==prematch_blue_data['gameid'][i]) &
                                   (data['side']=='Blue')]
            red_game_slice = data[(data['gameid']==prematch_red_data['gameid'][i]) &
                                  (data['side']=='Red')]
...
# clean
prematch_data = clean_raw_data(all_data)
